...
/*
        multiSites = [
        { "name" : "Site 4", "parent":"Daughter of A", "group":1, "no_parent": true },
        { "name" : "Site 5", "parent":"Daughter of A", "group":1, "no_parent": true },
        { "name" : "Site 6", "parent":"Daughter of A", "group":1, "no_parent": true }
        ];
    var alt = svg.selectAll("g.sibling").data(multiSites).enter().append("g")
        .attr("class", "node")
        .attr("transform", function(d) { return "translate("+ d.x +","+ d.y +")"+ "rotate("+ 0 +")"; });
    alt.append("rect")
     .attr('class', "recBox")
    ...
    alt.append("text")
     ...
     .text( function(d) { if (isRealValue(d.name)) return d.name; else return d; } );
...
if (treeOrientation == "vertical") {
    svg.attr("transform", "translate("+ (250+margin.left)+","+(100+margin.right)+")"+" rotate(90) scale(1)")
    .call(zoom);
} else {
